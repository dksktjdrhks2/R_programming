...
hist(t$Y16_CNT, xlim = c(0, 100), ylim = c(0,10), main ='17_CNT분포') 

# t 데이터 SEX 변수를 수량 정도를 막대 그래프 형태로 시각화하시오. 
freq(t$SEX, plot = T, main = 'SEX분포')

# t 데이터 SEX 변수를 수량 정도를 테이블을 생성하시오,
t1 <- table(t$SEX)
t1
# t1 데이터 SEX 변수에서 가로값의 범위(0~10),제목(BAR), 가로레이블(성별), 세로레이블(빈도수), 여성과 남성의 데이터 이름(여성, 남성)과 칼라(red, blue)의 추가하여 barplot 함수를 사용해서 시각화하시오. 
