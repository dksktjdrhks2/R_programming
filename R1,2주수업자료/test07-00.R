# reshape2패키지(행과 열을 전환)

install.packages("reshape2")
library(reshape2)

# melt(데이터 세트, id.var ='기준 열', measure.var = '변환 열') 함수
# 내장 데이터 세트인 airquality 불러 오시오
airquality 
# airquality의 변수명을 볼수있는 앞부분 6개를 불러 오시오
head(airquality)
#  # 기존에 있는 airquality의 변수명을  불러와 소문자로 대체하여 불러 오시오
names(airquality) <- tolower(names(airquality))  
# airquality의 바뀐 변수명을 볼수있는 앞부분 6개를 불러 오시오
head(airquality) 
# 기준 열을 미지정하여 모든 열을 t로 반환하고 앞과 뒷부분 6개를 불러 오시오
t <- melt(airquality)              
head(t)
tail(t)
# 기준값을 월과 오존으로하고, 변환열을 태양열로 하여 t로 반환하고, 앞과 뒷부분 6개를 불러 오시오
...
acast(t1, month ~ variable, mean) # 변수들을 월 별 평균으로 요약하기
# 변수들을 월 별 길이를 요약하여 불러 오시오.
acast(t1, month ~ variable, length)  # 변수들을 월 별 합으로 요약하기
